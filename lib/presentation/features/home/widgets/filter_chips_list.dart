...
class FilterChipsList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final genresAsync = ref.watch(isTv ? tvGenresProvider : movieGenresProvider);
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (filter.sortField != SortField.popularity || filter.sortOrder != SortOrder.descending)
            _buildChip(
              context,
              ref,
          ...
          // Availabilities
          ...filter.availabilities.map((a) => _buildChip(
                context,
                ref,
                'Type: $a',
                () {
          ...
          if (filter.userScore.start > 0 || filter.userScore.end < 10)
            _buildChip(
              ...
              () {
                _updateFilter(ref, filter.copyWith(userScore: const RangeValues(0, 10)));
              },
            ),

          // Release Types
          ...filter.releaseTypes.map((t) => _buildChip(
                context,
                ref,
          ...
          if (filter.releaseDateFrom != null || filter.releaseDateTo != null)
            _buildChip(
              ...
              () {
                _updateFilter(ref, filter.copyWith(releaseDateFrom: null, releaseDateTo: null));
              },
            ),
          ...
          ...filter.genres.map((gId) {
            final genreName = genresAsync.maybeWhen(
              data: (list) => list.firstWhere((g) => g.id == gId, orElse: () => const MovieGenre(id: 0, name: 'Unknown')).name,
              orElse: () => 'Genre $gId',
            );
            return _buildChip(
              context,
          ...
          if (filter.runtime.start > 0 || filter.runtime.end < 390)
            _buildChip(
              ...
              () {
                _updateFilter(ref, filter.copyWith(runtime: const RangeValues(0, 390)));
              },
            ),
        ],
      ),
  ...
  Widget _buildChip(BuildContext context, WidgetRef ref, String label, VoidCallback onDeleted) {
    return Padding(
      ...
      child: InputChip(
        ...
        side: BorderSide(color: AppColors.cinemaAccent.withValues(alpha: 0.3)),
      ),
    );
  }
