...
class MediaGrid extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final customFilter = ref.watch(isTv ? tvFilterProvider : movieFilterProvider);

    // Determine which data provider to use
    final provider = !customFilter.isDefault
        ? movieSectionProvider(isTv ? MovieSection.tvDiscover : MovieSection.discover)
        : genreId != null
            ? genreSectionProvider((id: genreId!, isTv: isTv))
            : movieSectionProvider(isTv ? MovieSection.tvPopular : MovieSection.popular);
          ...
          onNotification: (scrollInfo) {
            if (scrollInfo.metrics.pixels >=
              ...
              if (!customFilter.isDefault) {
                loadNextPages(ref, isTv ? MovieSection.tvDiscover : MovieSection.discover);
              } else if (genreId != null) {
                loadNextGenrePages(ref, genreId!, isTv: isTv);
              } else {
                loadNextPages(ref, isTv ? MovieSection.tvPopular : MovieSection.popular);
              }
            }
            return false;
          },
