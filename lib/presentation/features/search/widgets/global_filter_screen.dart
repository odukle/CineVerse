...
class GlobalFilterScreen extends ConsumerStatefulWidget {
  const GlobalFilterScreen({super.key});

  @override
  ConsumerState<GlobalFilterScreen> createState() => _GlobalFilterScreenState();
}

class _GlobalFilterScreenState extends ConsumerState<GlobalFilterScreen> {
  late GlobalMediaFilter _currentFilter;

  @override
  void initState() {
    super.initState();
    // Will be set in didChangeDependencies to access ref
  }

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    _currentFilter = ref.read(searchProvider).filter;
  }

  void _applyFilters() {
    ref.read(searchProvider.notifier).updateFilter(_currentFilter);
    Navigator.of(context).pop();
  }

  void _resetFilters() {
    setState(() {
      _currentFilter = const GlobalMediaFilter();
    });
  }
  ...
  Widget _buildMediaTypeChips() {
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: GlobalMediaType.values.map((type) {
        final isSelected = _currentFilter.mediaType == type;
        return FilterChip(
          ...
          onSelected: (selected) {
            if (selected) {
              setState(
                () => _currentFilter = _currentFilter.copyWith(mediaType: type),
              );
            }
          },
  ...
  Widget _buildReleaseTypeChips() {
    // Hide release types for TV-only mode
    if (_currentFilter.mediaType == GlobalMediaType.tv) {
  ...
  Widget _buildGenreChips() {
    // For global filter, we need to show both movie and TV genres
    ...
    return movieGenresAsync.when(
      data: (movieGenres) => tvGenresAsync.when(
        data: (tvGenres) {
          // Combine and deduplicate genres by ID
          final allGenres = <int, _GenreItem>{};
          for (final genre in movieGenres) {
            allGenres[genre.id] = _GenreItem(
              id: genre.id,
              name: genre.name,
              isMovie: true,
            );
          ...
          for (final genre in tvGenres) {
            if (allGenres.containsKey(genre.id)) {
              allGenres[genre.id] = _GenreItem(
                id: genre.id,
                name: genre.name,
                isMovie: true,
          ...
          final genres = allGenres.values.toList()
