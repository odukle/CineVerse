#!/usr/bin/env python3
"""One-time TV backfill from TMDB exports + TV details.

Creates/updates a TV dataset CSV (default: TMDB_tv_dataset.csv).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint-file", default=".local/tmdb_backfill_tv_checkpoint.json")
    parser.add_argument("--save-every", type=int, default=50)
    parser.add_argument("--log-every", type=int, default=100)
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--resume-db", default=".local/tmdb_backfill_tv_resume.sqlite")
    parser.add_argument("--working-db", default=".local/tmdb_backfill_tv_working.sqlite")
...
def main() -> None:
    ...
    LOGGER.info(
        "Backfill TV starting from index %s/%s using export %s%s",
        start_index,
        total_ids,
        export_name,
    ...
    try:
        ...
        if should_reuse_working_db:
            LOGGER.info("Reusing existing TV working DB: %s", working_db_path)
            existing_ids = load_existing_ids_from_sqlite(
                conn=conn,
                table_name="rows",
            )
        else:
            LOGGER.info("Loading existing TV CSV into working DB (if present): %s", csv_path)
        ...
        LOGGER.info("Loaded %s existing TV IDs", len(existing_ids))
        ...
        for idx in range(start_index, total_ids):
            tv_id = ids[idx]
            processed += 1
            if args.resume and has_resume_processed_id(resume_conn, run_key=run_key, row_id=tv_id):
                skipped_resumed += 1
                continue
            ...
            if details is None:
                ...
                if not row.get("id"):
                    ...
                    fetched += 1
                    was_existing = tv_id in existing_ids
        ...
        if args.in_place:
            ...
            LOGGER.info("TV backfill wrote %s rows and replaced %s", exported, csv_path)
        else:
            LOGGER.info("TV backfill wrote %s rows to %s", exported, output_csv)
        ...
        LOGGER.info("TV backfill finished in %s", format_duration(int(time.time() - started)))
    finally:
        conn.close()
        resume_conn.close()
