...
def main() -> None:
    ...
    LOGGER.info("Backfill TV starting from index %s/%s", start_index, total_ids)
    allowed_languages = {
        token.strip().lower()
    ...
    with tempfile.TemporaryDirectory(prefix="tmdb_tv_backfill_") as tmp_dir:
        ...
        try:
            ...
            LOGGER.info("Loading existing TV CSV into local DB (if present): %s", csv_path)
