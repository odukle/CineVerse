#!/usr/bin/env python3
"""Incremental TMDB TV sync using /tv/changes (14-day window aware)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--csv", default="TMDB_tv_dataset.csv")
    ...
    parser.add_argument("--checkpoint-file", default=".local/tmdb_incremental_tv_checkpoint.json")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Auto-derive start date from latest TV air date in dataset (minus buffer days).",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.start_date:
        ...
        if args.window_from_latest_release:
            latest_air = latest_date_from_csv(
                csv_path=Path(args.csv),
                date_columns=["last_air_date", "first_air_date"],
            )
            if latest_air is not None:
                if latest_air > today:
                    latest_air = today
                anchor = latest_air - dt.timedelta(days=max(0, args.window_buffer_days))
                candidates.append(anchor)
                LOGGER.info(
                    "Derived TV window anchor from latest air date=%s with buffer=%s day(s) -> %s",
                    latest_air,
    ...
    if not changed_ids:
        LOGGER.info("No changed TV IDs found for requested range.")
        save_checkpoint(
            checkpoint_path,
            {
    ...
    LOGGER.info("Total unique changed TV IDs: %s", len(changed_list))

    csv_path = Path(args.csv)
    with tempfile.TemporaryDirectory(prefix="tmdb_tv_incremental_") as tmp_dir:
        db_path = Path(tmp_dir) / "tv.sqlite"
        ...
        try:
            ...
            failed = 0
            skipped_existing = 0
            for tv_id in changed_list:
                processed += 1
                if args.insert_only and tv_id in existing_ids:
                    skipped_existing += 1
                    continue
                try:
                    ...
                    failed += 1
                    LOGGER.warning("Failed tv id=%s: %s", tv_id, error)
                    continue
                if details is None:
                    not_found += 1
                    continue
                row = tv_row_from_details(details)
                if not row.get("id"):
                    not_found += 1
                    continue
                upsert_row_sqlite(
            ...
            if args.in_place:
                ...
                LOGGER.info("Incremental TV sync replaced %s (%s rows)", csv_path, exported)
            else:
                LOGGER.info("Incremental TV sync wrote %s rows to %s", exported, output_csv)
            ...
            LOGGER.info("TV incremental sync finished in %s", format_duration(int(time.time() - started)))
        finally:
            conn.close()
